...
ECHOCARD_BATCH_SIZE="${ECHOCARD_BATCH_SIZE:-128}"
ECHOCARD_GRAD_ACCUM_STEPS="${ECHOCARD_GRAD_ACCUM_STEPS:-1}"
...
run_pretrain 1 echonet_echocardmae_repro configs/pretrain/stage0_echonet_echocardmae_400.yaml "$ECHOCARD_BATCH_SIZE" "$ECHOCARD_GRAD_ACCUM_STEPS"
run_pretrain 2 echonet_videomae_clean configs/pretrain/stage0_echonet_videomae_clean_400.yaml "$VIDEOMAE_BATCH_SIZE" "$VIDEOMAE_GRAD_ACCUM_STEPS"
...
if [[ "$RUN_FULL_FINETUNE_400" == "1" ]]; then
  RUN_TAG="$RUN_TAG"   PRETRAIN_ROOT="$PRETRAIN_ROOT"   ECHO_DATA_ROOT="$ECHO_DATA_ROOT"   NUM_WORKERS="$NUM_WORKERS"   PREFETCH_FACTOR="$PREFETCH_FACTOR"   bash scripts/eval_epoch400_full_downstream.sh
fi
